    ...
    public void configure(StateMachineTransitionConfigurer<EventState, EventStateMachineEvent> transitions) throws Exception {
        transitions
                ...
                .withExternal()
                    ...
                    .and()
                .withExternal()
                    .source(EventState.PLANNED).target(EventState.ARCHIVED)
                    .event(EventStateMachineEvent.CANCEL)
                    .action(actions.cancelAction())
                    .and()
                .withExternal()
                    .source(EventState.NOTIFIED).target(EventState.ARCHIVED)
                    .event(EventStateMachineEvent.CANCEL)
                    .action(actions.cancelAction())
                    .and()
                .withExternal()
                    .source(EventState.ALBUM_POSTED).target(EventState.ARCHIVED)
                    .event(EventStateMachineEvent.CANCEL)
                    .action(actions.cancelAction())
                    .and()
                .withExternal()
