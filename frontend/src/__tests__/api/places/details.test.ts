...
describe("GET /api/places/details", () => {
  ...
  it("calls Google with correct headers and returns 204", async () => {
    ...
    const res = await GET(
      new Request(`${BASE}?placeId=abc123&sessionToken=tok`),
    );
    expect(res.status).toBe(204);
    ...
    expect(
      (calledInit.headers as Record<string, string>)["X-Goog-Api-Key"],
    ).toBe("my-key");
    expect(
      (calledInit.headers as Record<string, string>)["X-Goog-FieldMask"],
    ).toBe("id");
    spy.mockRestore();
  });
